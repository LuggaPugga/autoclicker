...
pub fn keycode_from_string(s: &str) -> Option<Keycode> {
    match s.to_uppercase().as_str() {
        ...
        "LCTRL" | "LCONTROL" => Some(Keycode::LControl), "RCTRL" | "RCONTROL" => Some(Keycode::RControl),
...
pub fn check_hotkey(pressed_keys: &Vec<Keycode>, hotkey_string: &str) -> bool {
    ...
    if require_shift && !pressed_keys.contains(&Keycode::LShift) && !pressed_keys.contains(&Keycode::RShift) {
        return false;
    }
    if require_ctrl && !pressed_keys.contains(&Keycode::LControl) && !pressed_keys.contains(&Keycode::RControl) {
        return false;
    }
    if require_alt && !pressed_keys.contains(&Keycode::LAlt) && !pressed_keys.contains(&Keycode::RAlt) {
        return false;
    }
